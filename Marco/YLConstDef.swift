...
func YLImageOfFile(_ imageName: String) -> UIImage {
    let file = "\(Bundle.main.bundlePath)/\(imageName)"
    guard let image = UIImage(contentsOfFile: file) else { return UIImage() }
    return image.withRenderingMode(.alwaysOriginal)
}

func YLMainBundlePath(_ name: String, ofType: String?) -> String {
    return Bundle.main.path(forResource: name, ofType: ofType) ?? ""
}
