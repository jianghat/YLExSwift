...
extension UIButton {
    ...
    func setRightImageAndTextPadding(_ padding: CGFloat, labelWidth: CGFloat) {
        let image = self.image(for: .normal) ?? UIImage()
        self.titleEdgeInsets = UIEdgeInsets(top: 0, left: -image.size.width-padding, bottom: 0, right: image.size.width+padding)
        self.imageEdgeInsets = UIEdgeInsets(top: 0, left: labelWidth+padding, bottom: 0, right: 0)
        self.contentEdgeInsets = UIEdgeInsets(top: 0, left: padding, bottom: 0, right: padding)
    }
